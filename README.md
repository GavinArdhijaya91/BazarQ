...
Masuk dasbor penjual cukup dengan PIN booth 4–6 digit.

## Prinsip produk (dari PRD)

- **Ringan** — halaman pemesanan di bawah 100 KB, lancar di jaringan bazar yang padat.
- **Nol aplikasi** — berjalan di browser HP standar.
- **Satu layar satu keputusan** — kasir: lunas/belum. Dapur: masak/panggil.
- **Realtime** — semua perangkat di booth yang sama tersinkron otomatis via Firebase.

## Teknologi prototipe

- Frontend: HTML + CSS + JavaScript murni (tanpa build, tanpa framework).
- Backend: serverless — Firebase Realtime Database, satu namespace per booth (`bazarq/v2/booths/{slug}`).
